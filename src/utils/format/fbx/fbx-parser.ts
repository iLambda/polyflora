...
export type FBXPropertyType = FBXProperty['type'];
export type FBXPropertyValue<T extends FBXPropertyType> = (FBXProperty & { type: T })['value'];
export type FBXProperties<T extends FBXPropertyType> = { type: T, value: FBXPropertyValue<T> };
...
export type FBXProperty =
    ...
    | { type: 'i', value: Array<number> }
    | { type: 'f', value: Array<number> }
    | { type: 'd', value: Array<number> }
    | { type: 'l', value: Array<{ high: number, low: number }> }
...
const readPropertyArray: readPropertyArray = (isType, stream, count) => {
    ...
    if(isType('i')) {
        return { type: 'i', value: impl(() => stream.getInt32()) };
    } else if(isType('f')) {
        return { type: 'f', value: impl(() => stream.getFloat32()) };
    } else if(isType('d')) {
        return { type: 'd', value: impl(() => stream.getFloat64()) };
    } else if(isType('l')) {
        return { type: 'l', value: impl(() => ({
...
const propertyByteLength = (prop: FBXProperty): number => {
    if(prop.type === 'Y') {
        ...
        return 13 + prop.value.length * 4;
    } else if(prop.type === 'f') {
        return 13 + prop.value.length * 4;
    } else if(prop.type === 'd') {
        return 13 + prop.value.length * 8;
    } else if(prop.type === 'l') {
        return 13 + prop.value.length * 8;
    }
...
const serializeProperty = (prop: FBXProperty, offset: number, view: DataView): number => {
    ...
    if(prop.type == 'Y') {
        ...
        if(prop.type == 'f') compressedLength = prop.value.length * 4;
        else if(prop.type == 'd') compressedLength = prop.value.length * 8;
        else if(prop.type == 'l') compressedLength = prop.value.length * 8;
        else if(prop.type == 'i') compressedLength = prop.value.length * 4;
        else throw new Error('Invalid property');
        view.setUint32(offset+9, compressedLength, true);

        let i = 0;
        if(prop.type === 'l') for(const e of prop.value) {
            view.setUint32(offset + i*8 + 13, e.low, true);
            view.setUint32(offset + i*8 + 4 + 13, e.high, true);
            i++;
        } else for(const e of prop.value) {
